...
def cb_admin_check(func: Callable) -> Callable:
    async def decorator(client, cb):
        ...
        if cb.from_user.id in admemes:
            ...
            await cb.answer('You Bitch!!😏 not allow', show_alert=True)
            return
    return decorator                                                                       
...
async def play(_, message: Message):
    ...
    if message.chat.id in DISABLED_GROUPS:
        return    
    lel = await message.reply("**wait lemme process**")
    ...
    try:
        ...
        return
    text_links=None
    await lel.edit("lemme 🔍 find song for you")
    ...
    if audio:
        ...
        title = file_name
        thumb_name = "https://telegra.ph/file/cac99390201c9bec29674.jpg"
        ...
        query = toxt
        await lel.edit("wait lemme process")
        ...
        try:
            ...
            while j < 5:
                toxxt += f"{emojilist[j]} [Tɪᴛʟᴇ - {results[j]['title']}](https://youtube.com{results[j]['url_suffix']})\n"
...
async def deezer(client: Client, message_: Message):
    global que
    lel = await message_.reply("Wait lemme Play song 👨‍💻")
    administrators = await get_administrators(message_.chat)
    chid = message_.chat.id
    try:
        user = await USER.get_me()
    except:
        user.first_name =  "𝘾𝙊𝙁𝙁𝙄𝙉"
    usar = user
    wew = usar.id
    try:
        #chatdetails = await USER.get_chat(chid)
